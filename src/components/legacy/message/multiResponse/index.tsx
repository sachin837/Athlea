...
const MultiResponseMessageComp = ({
  TimeTextContent,
  MessageContent,
  animate = false,
  activity,
  ...
  const renderedContent = () => {
    switch (messageType) {
      ...
      case 'user':
        return <MessageText>{bodyText}</MessageText>;
      case 'userdata':
        if (graphType && typeof graphType === 'object' && graphType.data) {
          return (
            <GraphPlusHeaderGraph
              HeaderText={graphType.graphHeader}
              ScoreValue={graphType.graphScore}
              ScoreLabelText={graphType.graphLabel}
              graphType={graphType.graphType}
              data={graphType.data}
            />
          );
        } else {
          // Fallback content if graphType is not properly provided
  ...
  return (
    <View style={{paddingBottom: messageType === 'userdata' ? 0 : 30}}>
      {messageType === 'user' || messageType === 'usersource' ? (
        <UserMessageContainer>{renderedContent()}</UserMessageContainer>
      ) : messageType === 'userdata' ? (
        // This is the specific case for userdata
        // For example, to render a GraphPlusHeaderGraph without the UserMessageContainer styling
        <>{renderedContent()}</>
      ) : (
...
export default MultiResponseMessageComp;
