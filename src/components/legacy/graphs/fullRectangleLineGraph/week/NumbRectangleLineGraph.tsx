...
const NumbRectangleLineGraph = ({data: propData, svgWidthNumber = 350}) => {
  ...
  const yScale = d3
    .scaleLinear()
    .domain([0, d3.max(data, d => d.numb)])
    .range([svgHeight - rectHeight, 0])

  // Determine the min and max values for the value
  const minValue = d3.min(data, d => d.numb)
  const maxValue = d3.max(data, d => d.numb)
...
export default NumbRectangleLineGraph
