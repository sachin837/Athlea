...
const LoadGraph = ({data, widthPercentage = 0.9}) => {
  ...
  const animatedTSSHeights = data.map(d =>
    useSharedValue(d.selectedActivityLoad ? 0 : d.load),
  );
  ...
  useEffect(() => {
    ...
    animatedTSSHeights.forEach((animatedTSSHeight, index) => {
      ...
      const tssValue = data[index].selectedActivityLoad || 0;
  ...
  const selectedActivityTSSIndex = data.findIndex(
    d => d.selectedActivityLoad !== undefined,
  );
    ...
    <Svg width={svgWidth} height={svgHeight + 45}>
      <G>
        {data.map((item, index) => {
          ...
          const totalLoad = item.load + (item.selectedActivityLoad || 0);
          const barY = yScale(totalLoad);
          const barHeight = svgHeight - barY;
          const originalBarHeight = svgHeight - yScale(item.load);

          const tssHeight = item.selectedActivityLoad
            ? (item.selectedActivityLoad / item.load) * originalBarHeight
            : 0;
          ...
          const validAdditionalData = item.selectedActivityLoad || 0; // Fallback to 0 if additionalData is undefined
